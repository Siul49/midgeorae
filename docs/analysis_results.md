...
### 🧱 벽돌 카드 (Brick Cards - 위장 및 사기 시스템)
* **빌런의 핵심 도구**: 소지하지 않은 관전자나 구매자 화면에는 **일반 카테고리의 정상 물품으로 완벽히 마스킹되어 위장**됩니다.
* **항상 모든 물건 정보 공개 (Always Face-Up)**: 물건 카드 정보 공개 설정을 폐지하고 모든 물건 카드는 **항상 앞면이 보이는 상태로 고정**됩니다.
* **빌런조차 모르는 위장 (Blind Disguise)**: 벽돌 카드의 소유자(빌런/판매자)는 자기 화면에서 **[벽돌]**이라는 사실만 볼 수 있으며, 상대방에게는 어떤 물건으로 위장되어 있는지는 알 수 없습니다.
* **사기 메카닉**: 벽돌 카드는 게임 중에는 위장 가치를 그대로 자산에 유지하지만, 최종 정산 시 가치가 **0원**이 됩니다.
* **사기 거래의 단순화 (Scam Definition)**: 시세 가격과 상관없이 **"빌런이 벽돌 카드를 다른 플레이어에게 판매하는 것"** 자체를 사기 거래로 정의합니다.
* **매 라운드 사기 발생 알림 (Quest Result)**: 1사이클(라운드)이 끝날 때마다 해당 사이클 동안 사기 거래가 발생했는지 여부가 공개 로그를 통해 모든 플레이어에게 전달됩니다.

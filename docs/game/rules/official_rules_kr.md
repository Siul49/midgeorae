...
*   **시민 직업 종류**:
    ...
    | :--- | :--- | :--- |
    | **개발자 (Developer)** | 보유한 모든 물건의 카테고리가 '전자기기(Electronics)'여야 함 (최소 1개 이상 보유). | 1,500,000원 (첫 손패 감가 차감 전) |
    | **모델 (Model)** | 보유한 모든 물건의 카테고리가 '패션잡화(Fashion)'여야 함 (최소 1개 이상 보유). | 1,500,000원 (첫 손패 감가 차감 전) |
    | **주부 (Housewife)** | 보유한 모든 물건의 카테고리가 '생활용품(Living)'여야 함 (최소 1개 이상 보유). | 1,500,000원 (첫 손패 감가 차감 전) |
    | **벽돌 수집가 (Brick Collector)** | 게임 종료 시 '벽돌 카드'를 2개 이상 보유해야 함. | 1,500,000원 (첫 손패 감가 차감 전) |
    | **수집가 (Collector)** | 게임 종료 시 물건 카드(벽돌 포함)를 8개 이상 보유해야 함. | 1,500,000원 (첫 손패 감가 차감 전) |
    | **일반 시민 (General Citizen)** | 최종 점수 계산 시 총 자산이 2,000,000원 이상이어야 함. | 1,500,000원 (첫 손패 감가 차감 전) |
...
*   호스트가 방을 개설하고 플레이어가 참여합니다. (최소 3인 ~ 최대 5인, 기본 4인 추천)
...
| 카드명 | 액션 타입 | 설명 | 출현 가중치 |
| :--- | :--- | :--- | :--- |
| **구매 신청** | `tradeRequest` | 상대방의 패에 있는 물건 카드 중 하나를 선택해 가격을 제안하고 구매를 시도합니다. | 3 (약 21.4%) |
| **판매 신청** | `saleRequest` | 내 손패의 물건 카드 중 하나를 선택하여 가격을 매겨 상대방에게 제안합니다. | 3 (약 21.4%) |
| **무료 나눔** | `freeGive` | 내 손패의 물건 중 하나를 상대방에게 0원에 강제 양도하여 상대방 덱에 선물합니다. (거절 불가) | 2 (약 14.3%) |
| **직거래** | `directTrade` | 거래할 카드를 미리 상호 공개한 뒤 협상 테이블을 엽니다. | 2 (약 14.3%) |
| **악플 테러** | `badReview` | 지목한 플레이어의 평판 토큰 1개를 즉시 파괴합니다. | 2 (약 14.3%) |
| **분리수거** | `recycle` | 내 손패에 든 물건/벽돌 카드를 버려 소멸시킵니다. | 1 (약 7.1%) |
| **물물교환** | `swap` | 대상 플레이어와 무작위로 카드를 한 장씩 맞교환합니다. | 1 (약 7.1%) |

...
처음 로드맵에는 마피아식 지목에 가까운 표현이 있었지만, 중고거래 게임의 맥락에서는 "최종 신고"와 "분쟁 심사"가 더 자연스럽다. 그래서 GAME-2부터 다음 용어를 기준으로 삼는다.
...
이번 이슈의 범위는 시장 진행도와 신고 단계 진입까지다. 빌런 미션 완료, 증거 가중치, 직업 보너스는 GAME-7, GAME-8에서 다룬다.
...
- 최종 신고에서 증거는 단순 표시용이 아니라 신고 대상의 의심 점수에 가중치로 반영하는 방향을 기준으로 한다. 정확한 점수와 공개 범위는 GAME-7에서 테스트로 고정한다.
- 결과 화면에서 신고 집계, 팀 승패, 개인 점수를 어떻게 나눠 보여줄지는 GAME-8에서 결정한다.
- 기존 클라이언트 리듀서형 초안(`src/features/game/engine`, `components`)에는 낡은 라운드/지목 계열 용어가 남아 있다. 현재 `/game` 라우트는 온라인 서버형 `MidgeoraeOnlineGame`을 사용하므로, 이 초안은 후속 정리 이슈에서 제거하거나 용어를 맞춘다.
